...
This Helm chart deploys [Meridian](https://meridian.app) - an open source project management platform focused on simplicity and efficiency.

## Introduction

This chart bootstraps a Meridian deployment on a Kubernetes cluster using the Helm package manager. It deploys both the API backend and Web frontend components, with optional ingress resources.
...
To install the chart with the release name `my-meridian`:

```bash
helm install my-meridian ./charts/meridian
```

The command deploys Meridian on the Kubernetes cluster with default configuration. The [Parameters](#parameters) section lists the parameters that can be configured during installation.

## Uninstalling the Chart

To uninstall/delete the `my-meridian` deployment:

```bash
helm uninstall my-meridian
```
...
| `api.image.repository`              | API image repository                                                                                              | `ghcr.io/meridianhq/api`          |
...
| `api.persistence.dbFilename`        | Name of the SQLite database file                                                                                 | `meridian.db`                      |
...
| `web.image.repository`              | Web image repository                                                                                               | `ghcr.io/meridianhq/web`          |
...
```bash
kubectl apply -f meridian-gateway.yaml
```
