    ...
    case "development":
      return {
        ...
        excludePaths: [
          ...
          "/api/db/metrics",
        ], // Exclude frequent health checks
      };
    ...
    case "production":
      return {
        ...
        categoryFilters: process.env.LOG_CATEGORIES
          ? process.env.LOG_CATEGORIES.split(",").map(
              (c) => c.trim().toUpperCase() as LogCategory,
            )
          : [],
      };
...
export function getSecurityLoggingConfig(): SecurityLoggingConfig {
  return {
    ...
    alertThresholds: {
      failedLogins: Number.parseInt(process.env.FAILED_LOGIN_THRESHOLD || "5"),
      suspiciousRequests: Number.parseInt(
        process.env.SUSPICIOUS_REQUEST_THRESHOLD || "10",
      ),
      timeWindow: Number.parseInt(process.env.ALERT_TIME_WINDOW || "15"), // 15 minutes
    },
  };
}
...
export function getPerformanceLoggingConfig(): PerformanceLoggingConfig {
  return {
    ...
    metricsInterval: Number.parseInt(process.env.METRICS_INTERVAL || "60"), // 1 minute
  };
}
