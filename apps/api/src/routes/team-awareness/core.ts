...
teamAwareness.get(
  ...
  async (c) => {
    try {
      ...
      const topUsers = await ActivityTracker.getMostActiveUsers(
        workspaceId,
        limit,
      );
...
teamAwareness.post(
  ...
  async (c) => {
    try {
      const { userId, workspaceId, currentProjectId, currentTaskId } =
        c.req.valid("json");

      await UserStatusService.heartbeat(
        userId,
        workspaceId,
        currentProjectId,
        currentTaskId,
...
teamAwareness.get("/kudos/received/:userId", async (c) => {
  try {
    ...
    const kudos = await KudosService.getReceivedKudos(
      userId,
      workspaceId,
      limit,
    );
...
teamAwareness.get("/mood/user/:userId", async (c) => {
  try {
    ...
    const history = await MoodTrackerService.getUserMoodHistory(
      userId,
      workspaceId,
      days,
    );
...
teamAwareness.get("/skills/popular", async (c) => {
  try {
    ...
    const popularSkills = await SkillsService.getPopularSkills(
      workspaceId,
      limit,
    );
