...
import {
  ...
  roleAuditLog,
  roleTemplates,
} from "../database/schema/rbac-unified";
...
async function verifySystemRoles(): Promise<boolean> {
  try {
    ...
    const missingRoles = expectedSystemRoles.filter(
...
async function verifyRoleTemplates(): Promise<boolean> {
  try {
    ...
    addResult(
      "Role templates",
      true,
      `${templates.length} system templates present`,
    );
    return true;
  } catch (error) {
    addResult(
...
async function verifyRoleAssignments(): Promise<boolean> {
  try {
    ...
    const userCount = Number.parseInt(usersCheck.rows[0]?.count || "0");

    // Ideally, every user should have at least one role assignment
    const coveragePercent =
      userCount > 0 ? (assignments.length / userCount) * 100 : 0;
...
async function checkForOrphans(): Promise<boolean> {
  try {
    ...
    const orphanCount = Number.parseInt(
      orphanedAssignments.rows[0]?.count || "0",
    );
...
async function main() {
  ...
  try {
    ...
    await verifyTable("role_assignments", [
      "id",
      "user_id",
      "role_id",
      "workspace_id",
    ]);
    await verifyTable("permission_overrides", [
      "id",
      "user_id",
      "permission",
      "granted",
