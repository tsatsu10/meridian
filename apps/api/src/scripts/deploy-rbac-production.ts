...
function log(level: "info" | "success" | "warning" | "error", message: string) {
  ...
  logger.debug(
    `${colors[level]}[${timestamp}] [${level.toUpperCase()}] ${message}${reset}`,
  );
}
...
async function preFlightChecks(): Promise<boolean> {
  ...
  try {
    ...
    if (conflicts.rows && conflicts.rows[0]?.count > 0) {
      log(
        "warning",
        "⚠️  Tables already exist - may be a re-run or failed previous attempt",
      );
    }
...
async function createBackup(): Promise<string | null> {
  ...
  try {
    ...
    log(
      "success",
      `✅ Backup created: ${backupFile} (${(stats.size / 1024).toFixed(2)} KB)`,
    );
...
async function runMigrations(): Promise<boolean> {
  ...
  try {
    ...
    const migrations = [
      ...
      {
        file: "003_migrate_role_assignments.sql",
        desc: "Migrating role assignments",
      },
    ];
...
async function verifyMigration(): Promise<boolean> {
  ...
  try {
    ...
    log(
      "success",
      `✅ Role assignments present (${assignmentCount.rows?.[0]?.count || 0} assignments)`,
    );
    ...
    log(
      "success",
      `✅ Indexes created (${indexes.rows?.[0]?.count || 0} indexes)`,
    );
