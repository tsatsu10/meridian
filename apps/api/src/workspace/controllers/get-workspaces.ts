...
async function getWorkspaces(userEmail: string) {
  ...
  try {
    ...
    logger.debug(
      `👥 Found ${userWorkspaceMemberships.length} active workspace memberships for user ${userEmail}`,
    );

    if (userWorkspaceMemberships.length === 0) {
      logger.debug(
        `❌ No active workspace memberships found for user ${userEmail}`,
      );
      return [];
    }
    ...
    const workspaceIds = userWorkspaceMemberships.map(
    ...
    logger.debug(
      `🔍 Found ${workspaces.length} workspace records for user ${userEmail}`,
    );
    ...
    logger.debug(
      `✅ Returning ${results.length} workspaces for user ${userEmail}`,
    );
