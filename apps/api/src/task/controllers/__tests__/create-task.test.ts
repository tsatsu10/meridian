...
describe("CreateTask Controller", () => {
  ...
  describe("Task numbering", () => {
    it("should assign incremental task numbers", async () => {
      // Arrange
      const getNextTaskNumber = (await import("../get-next-task-number"))
        .default;
  ...
  describe("Error handling", () => {
    it("should handle database errors gracefully", async () => {
      ...
      // Act & Assert — the controller wraps unexpected errors in a 500
      // HTTPException rather than leaking raw database messages
      await expect(createTask(taskData)).rejects.toThrow(
