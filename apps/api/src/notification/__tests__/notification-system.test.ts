...
describe.skip("Notification System", () => {
  ...
  describe("Notification Queries", () => {
    ...
    it("should filter by notification type", async () => {
      ...
      expect(taskNotifications.every((n) => n.type === "task_assigned")).toBe(
        true,
      );
    });

    it("should filter by priority", async () => {
      const highPriorityNotifications = await db
        .select()
        .from(notificationTable)
        .where(
          and(
      ...
      expect(
        highPriorityNotifications.every((n) => n.priority === "high"),
      ).toBe(true);
    });
  ...
  describe("Notification Batching", () => {
    it("should group notifications by type", async () => {
      ...
      const taskAssignedNotifications = await db
        .select()
        .from(notificationTable)
        .where(
          and(
