...
export async function getProjectAnalytics(c: Context) {
  ...
  logger.debug(
    `📊 Analytics request for project: ${projectId}, timeRange: ${timeRange}`,
  );
  ...
  try {
    ...
    const avgCompletionTime =
      taskMetrics.completed > 0 ? totalHours / taskMetrics.completed : 0;
    ...
    const memberPerformance = await Promise.all(
      teamMembers.map(async (member) => {
        ...
        const totalMinutes = memberTimeEntries.reduce(
          (sum, entry) => sum + (entry.duration || 0),
          0,
        );
        const totalHours = totalMinutes / 60;
    ...
    for (let i = daysBack - 1; i >= 0; i--) {
      ...
      taskTrend.push({
        ...
        cumulative: allTasks.filter((t) => t.createdAt && t.createdAt <= date)
          .length,
    ...
    const historicalTasks = allTasks.filter(
      (t) =>
        t.createdAt &&
    ...
    const burndownData = [];
    const totalTasksAtStart = allTasks.filter(
      (t) => t.createdAt && t.createdAt <= startDate,
    ).length;
    ...
    for (let i = 0; i <= daysBack; i++) {
      ...
      const completedByDate = allTasks.filter(
        (t) => t.status === "done" && t.createdAt && t.createdAt <= date,
      ).length;

      const createdByDate = allTasks.filter(
        (t) => t.createdAt && t.createdAt <= date,
      ).length;
    ...
    const onTimeRate =
      taskMetrics.total > 0
        ? ((taskMetrics.total - taskMetrics.overdue) / taskMetrics.total) * 100
        : 100;
    const projectHealth = {
      score: Math.round(
        completionRate * 0.4 + onTimeRate * 0.3 + timeMetrics.efficiency * 0.3,
      ),
      riskFactors: [
