...
class LogAggregationService extends EventEmitter {
  ...
  private userActivity = new Map<
    string,
    { actions: number; lastSeen: number }
  >();
  ...
  recordLog(metrics: LogMetrics): void {
    ...
    if (metrics.userId) {
      const existing = this.userActivity.get(metrics.userId) || {
        actions: 0,
        lastSeen: 0,
      };
   ...
   */
  getAnalytics(timeWindowMs = 3600000): LogAnalytics {
    const now = Date.now();
    ...
    if (options.filters) {
      ...
      if (category) {
        filteredLogs = filteredLogs.filter((log) =>
          category.includes(log.category),
        );
      }
  ...
  getHealth(): {
    ...
    const errorRatePercentage =
      recentLogs.length > 0
        ...
        : 0;
    const alertRulesActive = this.alertRules.filter(
      (rule) => rule.enabled,
    ).length;
  ...
  private initializeDefaultAlerts(): void {
    ...
    this.alertRules.push({
      ...
      condition: (metrics) =>
        metrics.duration !== undefined && metrics.duration > 5000, // 5 seconds
      severity: "medium",
