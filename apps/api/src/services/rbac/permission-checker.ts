...
export class PermissionChecker {
  ...
  async checkPermission(
    ...
    try {
      ...
      const override = await this.checkPermissionOverride(
        userId,
        permission,
        context,
      );
      if (override !== null) {
        return {
          ...
          reason:
            override.reason ||
            (override.granted ? "Explicitly granted" : "Explicitly revoked"),
          source: "override",
        };
      }
    ...
    try {
      ...
      for (const assignment of assignments) {
        const role = await this.db
          .select()
          .from(roles)
          .where(and(eq(roles.id, assignment.roleId), eq(roles.isActive, true)))
          .limit(1);
  ...
  async checkPermissions(
    ...
    for (const permission of permissions) {
      ...
      const override = userPerms.overrides.find(
    ...
    try {
      ...
      const sortedRoles = [...userPerms.roles].sort((a, b) => {
        if (a.type === "system" && b.type === "system") {
          return (
            (systemRolePriority[a.id] || 999) -
            (systemRolePriority[b.id] || 999)
          );
        }
