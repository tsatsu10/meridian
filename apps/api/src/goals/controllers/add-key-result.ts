...
export async function addKeyResult(c: Context) {
  try {
    ...
    if (!body.unit) {
      return c.json(
        { error: "Unit is required (%, count, currency, hours, custom)" },
        400,
      );
    }
    ...
    if (!["%", "count", "currency", "hours", "custom"].includes(body.unit)) {
      return c.json(
        {
          error: "Invalid unit. Must be: %, count, currency, hours, or custom",
        },
        400,
      );
    }
    ...
    if (goal.userId !== userId) {
      return c.json(
        { error: "You can only add key results to your own goals" },
        403,
      );
    }
