...
export async function createGoal(c: Context) {
  try {
    ...
    if (!body.type) {
      return c.json(
        { error: "Type is required (objective, personal, team, or strategic)" },
        400,
      );
    }
    ...
    if (!["objective", "personal", "team", "strategic"].includes(body.type)) {
      return c.json(
        {
          error:
            "Invalid type. Must be: objective, personal, team, or strategic",
        },
        400,
      );
    }
    ...
    if (!body.timeframe) {
      return c.json(
        { error: "Timeframe is required (e.g., 'Q1 2025', '2025', 'custom')" },
        400,
      );
    }
