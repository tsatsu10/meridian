...
export async function updateKeyResult(c: Context) {
  try {
    ...
    if (goalRow.userId !== userId) {
      return c.json(
        { error: "You can only update key results for your own goals" },
        403,
      );
    }

    // Track if currentValue changed for progress logging
    const oldValue = Number.parseFloat(existingKR.currentValue as string);
    const newValue =
      body.currentValue !== undefined ? body.currentValue : oldValue;
    ...
    if (body.targetValue !== undefined)
      updateData.targetValue = body.targetValue.toString();
    if (body.currentValue !== undefined)
      updateData.currentValue = body.currentValue.toString();
...
async function recalculateGoalProgress(goalId: string) {
  try {
    ...
    for (const kr of keyResults) {
      const current = Number.parseFloat(kr.currentValue as string) || 0;
      const target = Number.parseFloat(kr.targetValue as string) || 1;
    ...
    const isNowCompleted =
      averageProgress >= 100 && completedCount === keyResults.length;
