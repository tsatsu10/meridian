...
async function inviteWorkspaceUser(
  ...
  const inviterRoleLevel =
    ROLE_HIERARCHY[inviterMembership.role as keyof typeof ROLE_HIERARCHY] || 0;
  const assignedRoleLevel =
    ROLE_HIERARCHY[assignedRole as keyof typeof ROLE_HIERARCHY] || 0;
  ...
  const inviterName =
    inviter?.name ?? inviterEmail.split("@")[0] ?? "Workspace member";
  ...
  return {
    ...invitedMember,
    roleName:
      AVAILABLE_ROLES.find((r) => r.value === assignedRole)?.label ||
      assignedRole,
  };
}
