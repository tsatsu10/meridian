...
export async function removeMember(c: Context) {
  ...
  if (!workspaceId || !memberId || !currentUserEmail) {
    return c.json(
      { error: "workspaceId, memberId, and authenticated user are required" },
      400,
    );
  }

  logger.debug(
    `🗑️ Remove member request: workspace=${workspaceId}, member=${memberId}, by=${currentUserEmail}`,
  );
  ...
  try {
    ...
    const currentUserHierarchy =
      ROLE_HIERARCHY[currentUser.role as string] || 0;
    const targetMemberHierarchy =
      ROLE_HIERARCHY[targetMember.role as string] || 0;

    if (currentUserHierarchy <= targetMemberHierarchy) {
      logger.debug(
        `❌ Insufficient permissions: currentUser=${currentUserHierarchy}, targetMember=${targetMemberHierarchy}`,
      );
      return c.json(
        {
    ...
    if (!currentUser.id) {
      return c.json(
        { error: "Current user linkage is invalid for member removal" },
        400,
      );
    }
    ...
    const ownedProjects = workspaceProjects.filter(
      (p) => p.ownerId === targetMember.userId,
    );
