...
export async function changeMemberRole(c: Context) {
  ...
  logger.debug(
    `🔄 Role change request: workspace=${workspaceId}, member=${memberId}, newRole=${role}, by=${currentUserEmail}`,
  );
  ...
  try {
    ...
    const currentUserHierarchy =
      ROLE_HIERARCHY[currentUser.role as ValidRole] || 0;
    const newRoleHierarchy = ROLE_HIERARCHY[role as ValidRole];
    const targetCurrentHierarchy =
      ROLE_HIERARCHY[targetMember.currentRole as ValidRole] || 0;

    // Must have higher role than the role being assigned
    if (currentUserHierarchy <= newRoleHierarchy) {
      logger.debug(
        `❌ Insufficient permissions: currentUser=${currentUserHierarchy}, newRole=${newRoleHierarchy}`,
      );
      return c.json(
        {
    ...
    if (!targetMember.userId || !currentUser.id) {
      return c.json(
        { error: "Member linkage is invalid for role updates" },
        400,
      );
    }
