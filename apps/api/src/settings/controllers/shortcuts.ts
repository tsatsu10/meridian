...
export interface ShortcutPreset {
  ...
  shortcuts: Omit<
    KeyboardShortcut,
    "id" | "workspaceId" | "userId" | "isCustom" | "createdAt" | "updatedAt"
  >[];
}
  ...
  },
  {
    ...
    shortcuts: [
      ...DEFAULT_SHORTCUTS.filter(
        (s) => !["g+d", "g+p", "g+t", "g+c", "g+s"].includes(s.shortcutKeys),
      ),
      {
        action: "navigate.dashboard",
        shortcutKeys: "Ctrl+1",
...
export async function getShortcuts(
  ...
  logger.info(
    `Fetching shortcuts for user ${userId} in workspace: ${workspaceId}`,
  );
...
export async function updateShortcuts(
  ...
  const db = getDatabase();
  logger.info(
    `Updating shortcuts for user ${userId} in workspace: ${workspaceId}`,
  );
...
export async function updateShortcut(
  ...
  shortcutId: string,
  updates: Partial<
    Omit<KeyboardShortcut, "id" | "workspaceId" | "userId" | "createdAt">
  >,
): Promise<KeyboardShortcut> {
  const db = getDatabase();
...
export async function resetShortcuts(
  ...
  const db = getDatabase();
  logger.info(
    `Resetting shortcuts for user ${userId} in workspace: ${workspaceId}`,
  );
