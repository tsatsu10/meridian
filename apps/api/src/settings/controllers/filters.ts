...
export const FILTER_TEMPLATES: FilterTemplate[] = [
  ...
  // Task filters
  {
    ...
    filterConfig: {
      logic: "AND",
      conditions: [{ field: "assignee", operator: "=", value: "$currentUser" }],
    },
  },
  {
    id: "tasks-overdue",
  ...
  },
  {
    ...
    filterConfig: {
      logic: "AND",
      conditions: [{ field: "updatedAt", operator: ">", value: "$7daysAgo" }],
    },
  },
  {
  ...
  // Message filters
  {
    ...
    filterConfig: {
      logic: "AND",
      conditions: [{ field: "isRead", operator: "=", value: false }],
    },
  },
  {
    ...
    filterConfig: {
      logic: "AND",
      conditions: [{ field: "hasAttachments", operator: "=", value: true }],
    },
  },
];
...
export async function getSavedFilters(
  ...
  logger.info(
    `Fetching saved filters for user ${userId} in workspace: ${workspaceId}`,
  );
...
export async function createSavedFilter(
  ...
  const db = getDatabase();
  logger.info(
    `Creating saved filter for user ${userId} in workspace: ${workspaceId}`,
  );
...
export async function updateSavedFilter(
  ...
  updates: Partial<
    Omit<
      SavedFilter,
      "id" | "workspaceId" | "userId" | "createdAt" | "usageCount"
    >
  >,
): Promise<SavedFilter> {
  const db = getDatabase();
