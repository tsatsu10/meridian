...
export async function getPublicProfile(c: Context) {
  try {
    ...
    const goalsWhere = isOwnProfile
      ? and(eq(goals.userId, targetUserId), eq(goals.status, "active"))
      : and(
    ...
    const completedGoalsCount = await db
      ...
      .from(goals)
      .where(
        and(eq(goals.userId, targetUserId), eq(goals.status, "completed")),
      );
    ...
    const goalsStats = {
      ...
      completionRate:
        completedCount > 0
          ? Math.round(
              (completedCount / (userGoals.length + completedCount)) * 100,
            )
          : 0,
    };
