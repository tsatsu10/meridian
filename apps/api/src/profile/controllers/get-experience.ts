...
const getExperience = async (userId: string) => {
  try {
    ...
    logger.debug("🔍 getExperience: Query complete", {
      rows: experience.length,
    });
    logger.debug("🔍 getExperience: Mapping results...");

    const mapped = experience.map((exp) => {
      logger.debug("🔍 getExperience: Mapping exp", {
        id: exp.id,
        skillsType: typeof exp.skills,
      });
      return {
        ...exp,
