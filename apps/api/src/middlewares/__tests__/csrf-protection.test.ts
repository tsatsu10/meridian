...
describe("CSRF Protection Middleware", () => {
  ...
  describe("HTTP method handling", () => {
    it("should skip CSRF check for GET requests", () => {
      ...
      const requiresCsrf = !["GET", "HEAD", "OPTIONS"].includes(
        mockContext.req.method,
      );
  ...
  describe("Error responses", () => {
    it("should return 403 for missing token", () => {
      ...
      const response = mockContext.json({ error: "CSRF token missing" }, 403);
    ...
    it("should return 403 for invalid token", () => {
      ...
      const response = mockContext.json({ error: "Invalid CSRF token" }, 403);
