 ...
 */
export function httpCacheHeaders(maxAge = 300): MiddlewareHandler {
  return async (c, next) => {
    await next();
 ...
 */
export function staticAssetCache(maxAge = 31536000): MiddlewareHandler {
  return async (c, next) => {
    await next();
    ...
    const isStatic =
      /\.(js|css|png|jpg|jpeg|gif|webp|svg|woff|woff2|ttf|eot|ico)$/.test(path);
