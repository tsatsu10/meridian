...
export const goals = pgTable(
  ...
  (table) => ({
    // Performance Indexes
    workspaceUserIdx: index("idx_goals_workspace_user").on(
      table.workspaceId,
      table.userId,
    ),
...
export const goalProgress = pgTable(
  "goal_progress",
  {
    ...
    keyResultId: text("key_result_id").references(() => goalKeyResults.id, {
      onDelete: "cascade",
    }),
...
export const goalReflections = pgTable(
  ...
  (table) => ({
    // Performance Indexes
    userWeekIdx: index("idx_reflections_user_week").on(
      table.userId,
      table.weekOf,
    ),
...
export const goalMilestones = pgTable(
  ...
  (table) => ({
    ...
    userStatusIdx: index("idx_milestones_user_status").on(
      table.userId,
      table.status,
    ),
...
export const goalKeyResultsRelations = relations(
  goalKeyResults,
  ({ one, many }) => ({
    goal: one(goals, {
...
export const goalReflectionsRelations = relations(
  goalReflections,
  ({ one }) => ({
    user: one(users, {
