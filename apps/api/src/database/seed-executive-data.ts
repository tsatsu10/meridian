    ...
    for (let i = 0; i < Math.min(projectCount, projectTemplates.length); i++) {
      ...
      const [project] = await db
        ...
        .values({
          ...
          actualSpent: Math.floor(
            template.budget * (0.6 + Math.random() * 0.3),
          ), // 60-90% spent
          createdAt: startDate,
          updatedAt: now,
        })
      ...
      logger.debug(
        `  ✓ Created project: ${project.name} (Health: ${template.health}%)`,
      );
      ...
      const tasksToCreate = Math.floor(
        tasksPerProject * (0.8 + Math.random() * 0.4),
      );
