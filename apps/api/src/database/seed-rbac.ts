...
export async function seedRBACData() {
  ...
  try {
    ...
    if (workspace.length) {
      ...
      logger.debug(
        "📍 No existing workspace found - users will create their own",
      );
    }
    ...
    for (const userData of demoUsers) {
      ...
      if (isWorkspaceOwner || userData.email === "admin@meridian.app") {
        ...
        if (!existingRole.length) {
          ...
          logger.debug(
            `    🛡️  Assigned ${userData.role} role to workspace owner: ${userData.name}`,
          );
        } else {
          logger.debug(
            `    ⏭️  Role already assigned for workspace owner: ${userData.name}`,
          );
        }
      } else {
        logger.debug(
          ...
          try {
            ...
            if (headUser.length) {
              ...
              if (departmentId) {
                ...
                logger.debug(
                  `  👑 Set ${dept.headEmail} as head of ${dept.name}`,
                );
              }
            }
          } catch (error) {
            logger.debug(
              `  ⚠️  Skipped setting head for ${dept.name}: ${error.message}`,
            );
          }
        }
      }
    ...
    const assignedUsers = demoUsers.filter(
      (u) =>
    ...
    logger.debug(
      `  🛡️  Role assignments: ${assignedUsers.length} (workspace owners only)`,
    );
    logger.debug(
      `  🔒 Security: ${demoUsers.length - assignedUsers.length} users must be invited to access workspaces`,
    );
    ...
    logger.debug("\n✅ SECURITY STATUS:");
    logger.debug(
      "  🔒 Users can only access workspaces they own or are invited to",
    );
...
export async function getRBACStats() {
  ...
  await initializeDatabase();
  const db = getDatabase();
  const [totalUsers, activeRoles, departments, roleHistory] = await Promise.all(
    [
  ...
  return {
    ...
    roleDistribution: activeRoles.reduce(
      (acc, role) => {
