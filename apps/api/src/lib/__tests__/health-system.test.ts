    ...
    it("handles health check creation", async () => {
      ...
      const response = await app.request("/health/checks", {
        ...
        body: JSON.stringify({
          name: "database",
          check:
            '() => Promise.resolve({ status: "HEALTHY", message: "Database is healthy" })',
          interval: 30000,
        }),
      });
    ...
    it("handles missing health data", () => {
      expect(() => {
        healthService.createCheck({
          name: "",
          check: () =>
            Promise.resolve({ status: HealthStatus.HEALTHY, message: "" }),
          interval: 0,
        });
      }).not.toThrow();
    });
