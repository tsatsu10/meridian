...
export class AppError extends Error {
  ...
  constructor(
    ...
    const isNewSignature = Object.values(ErrorCode).includes(
      codeOrMessage as ErrorCode,
    );

    if (
      !isNewSignature &&
      (messageOrStatusCode === undefined ||
      ...
      operational =
        typeof detailsOrIsOperational === "boolean"
          ? detailsOrIsOperational
          : true;
      code = "AppError";
    } else {
...
export function errorHandler() {
  return async (err: Error, c: Context) => {
    ...
    if (err instanceof AppError) {
      ...
      const code =
        typeof err.code === "string" && err.code in ErrorCode
          ? err.code
          : "AppError";
...
export const errorsRoute = new Hono().post(
  "/report",
  async (c, next) => {
