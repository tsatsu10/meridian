...
test.describe("Task Card - Real End-to-End Tests", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    if (await firstProject.isVisible()) {
      ...
      await page.fill(
        '[name="description"]',
        "Test project for E2E task card testing",
      );
  ...
  test.describe("Task Card Rendering", () => {
    ...
    test("should display assignee information with real user data", async ({
      page,
    }) => {
      const taskCard = page.locator('[role="article"]').first();
      ...
      const assigneeSection = taskCard.locator(
    ...
    test("should display priority indicator with correct color", async ({
      page,
    }) => {
      const taskCard = page.locator('[role="article"]').first();
      await expect(taskCard).toBeVisible();
      ...
      const priorityIndicator = taskCard
        .locator("svg.lucide-flag")
        .locator("..");
    ...
    test("should display subtask count if task has subtasks", async ({
      page,
    }) => {
      // Create a task with subtasks or find one
      const taskWithSubtasks = page
        .locator(
          '[role="article"]:has(svg.lucide-chevron-right, svg.lucide-chevron-down)',
        )
        .first();

      if (await taskWithSubtasks.isVisible()) {
        // Should show subtask count
        const subtaskButton = taskWithSubtasks.locator(
          "button:has(svg.lucide-chevron-right, svg.lucide-chevron-down)",
        );
    ...
    test("should handle unassigned tasks gracefully", async ({ page }) => {
      ...
      const unassignedCard = page
        .locator('[role="article"]:has([title="Unassigned"])')
        .first();
  ...
  test.describe("Task Card Interactions", () => {
    ...
    test("should open task details with keyboard (Enter key)", async ({
      page,
    }) => {
      const taskCard = page.locator('[role="article"]').first();
      await expect(taskCard).toBeVisible();
    ...
    test("should open task details with keyboard (Space key)", async ({
      page,
    }) => {
      const taskCard = page.locator('[role="article"]').first();
      await expect(taskCard).toBeVisible();
    ...
    test("should expand and collapse subtasks", async ({ page }) => {
      const taskWithSubtasks = page
        .locator('[role="article"]:has(button:has(svg.lucide-chevron-right))')
        .first();

      if (await taskWithSubtasks.isVisible()) {
        // Click to expand
        const expandButton = taskWithSubtasks
          .locator("button:has(svg.lucide-chevron-right)")
          .first();
        await expandButton.click();

        // Should show chevron-down icon (expanded state)
        const collapseButton = taskWithSubtasks.locator(
        ...
        const rightChevron = taskWithSubtasks.locator(
  ...
  test.describe("Task Card Drag and Drop", () => {
    test("should support drag and drop to change status", async ({ page }) => {
      ...
      if (await todoTask.isVisible()) {
        ...
        const dragHandle = todoTask.locator(
          '[aria-label="Drag to reorder task"]',
        );
    ...
    test("should show visual feedback during drag", async ({ page }) => {
      ...
      if (box) {
        ...
        const opacity = await taskCard.evaluate(
  ...
  test.describe("Task Card Edge Cases", () => {
    test("should handle very long task titles", async ({ page }) => {
      ...
      const longTitle =
        "This is a very long task title that should be truncated to prevent layout issues and maintain a clean UI appearance in the kanban board view";
      ...
      // Find the task card
      const taskCard = page
        .locator('[role="article"]:has-text("This is a very long")')
        .first();
    ...
    test("should handle tasks with dependencies", async ({ page }) => {
      ...
      const taskWithDeps = page
        .locator(
          '[role="article"]:has([class*="blue-50"], [class*="orange-50"])',
        )
        .first();

      if (await taskWithDeps.isVisible()) {
        // Should show dependency indicator
        const depsIndicator = taskWithDeps.locator(
          '[class*="blue-50"], [class*="orange-50"]',
        );
  ...
  test.describe("Task Card Performance", () => {
    ...
    test("should handle large number of tasks without performance degradation", async ({
      page,
    }) => {
      await page.goto("/dashboard/projects");
      const firstProject = page.locator('[data-testid="project-card"]').first();
