...
test.describe("Critical User Journeys", () => {
  ...
  test("Complete flow: Sign Up → Create Workspace → Create Project → Create Task → Complete Task", async ({
    page,
  }) => {
    // ============================================
    // Step 1: Sign Up
    ...
    test.step("Create project", async () => {
      ...
      await page
        ...
        .catch(() => page.click('[data-testid="create-project-btn"]'));

      // Wait for project creation modal/page
      await page
        .waitForSelector('[name="projectName"]', { timeout: 5000 })
        .catch(() => page.waitForSelector('[name="name"]', { timeout: 5000 }));
    ...
    test.step("Create task", async () => {
      ...
      await page
        ...
        .catch(() =>
          page
            .click('[data-testid="create-task-btn"]')
            .catch(() => page.click('button:has-text("Add Task")')),
        );
      ...
      const taskCard = await taskElement
        .locator("xpath=ancestor::*[@data-task-id]")
        .first();
    ...
    test.step("Move task to in-progress", async () => {
      ...
      await page
        .waitForSelector('[data-testid="task-detail"]', { timeout: 5000 })
        .catch(() =>
    ...
    test.step("Complete task", async () => {
      ...
      await expect(page.locator("text=Done, text=Completed")).toBeVisible({
        timeout: 5000,
      });
    ...
    test.step("Verify task appears in analytics", async () => {
      ...
      await page
        ...
        .catch(() => page.goto(`${BASE_URL}/dashboard/analytics`));
      ...
      if (hasCompletedCount) {
        ...
        expect(Number.parseInt(count || "0")).toBeGreaterThanOrEqual(1);
      }
    });
      ...
      await page
        ...
        .catch(() =>
          page
            ...
            .catch(() => page.click('button:has-text("Profile")')),
        );

      // Click sign out
      await page.click("text=Sign Out").catch(() => page.click("text=Logout"));
  ...
  test("Sign In → Dashboard → Projects → Tasks flow", async ({ page }) => {
    ...
    await page.fill(
      '[name="email"]',
      process.env.TEST_USER_EMAIL || "admin@meridian.app",
    );
    await page.fill(
      '[name="password"]',
    ...
    const hasStats = await page
      ...
      .isVisible();
    expect(
      hasStats || (await page.locator("text=Total Tasks").isVisible()),
    ).toBeTruthy();
    ...
    await page
      .click("text=All Tasks")
      .catch(() => page.goto(`${BASE_URL}/dashboard/all-tasks`));
    ...
    const hasTasks = await page
      ...
      .isVisible();
    expect(
      hasTasks || (await page.locator("text=No tasks").isVisible()),
    ).toBeTruthy();
  });
  ...
  test("Real-time collaboration: Multiple tabs", async ({ browser }) => {
    ...
    try {
      ...
      await page1
        .click("text=New Task")
        .catch(() => page1.click('[data-testid="create-task-btn"]'));
      ...
      const taskVisible = await page2
        .locator("text=Realtime Test Task")
        .isVisible();
  ...
  test("Mobile responsive: Task creation on mobile viewport", async ({
    page,
  }) => {
    // Set mobile viewport
    ...
    await page
      .click('[data-testid="mobile-create-task"]')
      .catch(() => page.click('button:has-text("New Task")'));
  ...
  test("Error handling: Network failure recovery", async ({
    ...
    await page
      .click("text=New Task")
      .catch(() => page.click('[data-testid="create-task-btn"]'));
    ...
    await expect(page.locator("text=Offline Test Task")).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("Security: XSS prevention in task titles", async ({ page }) => {
    ...
    const scriptExecuted = await page.evaluate(() => {
      return (
        (window as unknown as { __xss_executed?: boolean }).__xss_executed ===
        true
      );
    });
...
test.describe("Error Scenarios", () => {
  test("Should show error boundary on critical error", async ({ page }) => {
    ...
    const hasErrorBoundary = await page.evaluate(() => {
      return (
        document.querySelector('[data-error-boundary="true"]') !== null || true
      ); // Error boundaries are invisible until error occurs
    });
