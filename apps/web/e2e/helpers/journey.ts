...
export async function signUpViaUi(page: Page, user: TestUser) {
  ...
  const nameField = page
    .locator('[name="name"], input[placeholder*="full name" i]')
    .first();
  await expect(
    nameField,
    `Sign up form did not render on ${page.url()}`,
  ).toBeVisible({
    timeout: 15_000,
  });
  ...
  await page
    .locator('[name="email"], input[type="email"]')
    .first()
    .fill(user.email);
  await page
    .locator('[name="password"], input[type="password"]')
    .nth(0)
    .fill(user.password);
  await page
    .locator('[name="confirmPassword"], input[type="password"]')
    .nth(1)
    .fill(user.password);
  ...
  const signUpResponsePromise = page.waitForResponse((res) => {
    return (
      res.url().includes("/api/users/sign-up") &&
...
export async function ensureWorkspaceExists(page: Page) {
  ...
  if (
    ...
    await page
      .locator('[name="description"]')
      .fill("Workspace created by Playwright");
  }
...
export async function inviteWorkspaceUserViaApi(
  ...
  const res = await request.post(
    `/api/workspace-user/${encodeURIComponent(args.workspaceId)}/invite`,
    {
