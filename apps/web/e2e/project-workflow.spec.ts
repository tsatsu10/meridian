...
test.describe("Project Workflow E2E", () => {
  ...
  test.describe("Project Creation Workflow", () => {
    ...
    test("should navigate to project details", async ({ page }) => {
      ...
      // Click first project
      await page.click(
        '.project-card:first-child, [data-testid="project-card"]:first-child',
      );
  ...
  test.describe("Task Management Workflow", () => {
    ...
    test("should create new task", async ({ page }) => {
      ...
      if (projectExists) {
        ...
        // Fill task form
        await page.fill(
          'input[name="title"], input[placeholder*="task" i]',
          "E2E Test Task",
        );
    ...
    test("should move task between columns", async ({ page }) => {
      const hasKanban =
        (await page
          .locator('.kanban-column, [data-testid="kanban-column"]')
          .count()) > 0;
    ...
    test("should update task status", async ({ page }) => {
      const hasTask =
        (await page.locator('.task-card, [data-testid="task-card"]').count()) >
        0;

      if (hasTask) {
        // Click task to open details
        await page.click(
          '.task-card:first-child, [data-testid="task-card"]:first-child',
        );

        // Look for status dropdown or buttons
        const statusElement = page.locator(
          'select[name="status"], [aria-label*="status" i]',
        );
  ...
  test.describe("Dashboard Interactions", () => {
    ...
    test("should navigate between dashboard sections", async ({ page }) => {
      ...
      const navLinks = ["Projects", "Tasks", "Team", "Analytics"];
