...
export const logBundleAnalysis = () => {
  if (process.env.NODE_ENV === 'development') {
    ...
    logger.info("Critical Path Size:", analysis.criticalPath, "KB");
    logger.info("Lazy Loaded Size:", analysis.lazyLoaded, "KB");
    logger.info("Total Bundle Size:", analysis.total, "KB");
    logger.info("Bundle Efficiency:", analysis.efficiency, "%");

    console.group('🎯 Performance Budget Status');
    logger.info("Critical Path:", budgetResults.criticalPath.status,
                `(${budgetResults.criticalPath.actual}KB / ${budgetResults.criticalPath.target}KB)`);
    logger.info("Lazy Loaded:", budgetResults.lazyLoaded.status,
