...
function calculateCriticalPath(tasks: GanttTask[]): Set<string> {
  ...
  function calculateLatestTimes(taskId: string): number {
    ...
    let minDependentStart = Number.POSITIVE_INFINITY;
...
function GanttChart({ tasks }: GanttChartProps) {
  ...
  const { ganttTasks, criticalPath, timelineStart, timelineEnd, totalDays } =
    useMemo(() => {
      try {
        // Convert scheduled tasks to Gantt format
        const converted: GanttTask[] = scheduledTasks
          .map((scheduledTask) => {
        ...
        if (converted.length > 0) {
          const earliestTaskDate = new Date(
            Math.min(...converted.map((t) => t.startDate.getTime())),
          );
          const latestTaskDate = new Date(
            Math.max(...converted.map((t) => t.endDate.getTime())),
          );
  ...
  const timelineHeaders = useMemo(() => {
    ...
    while (currentHeaderDate <= timelineEnd) {
      headers.push({
        ...
        isToday:
          format(currentHeaderDate, "yyyy-MM-dd") ===
  ...
  const handleTodayClick = () => {
    ...
    if (timelineRef.current && totalDays > 0) {
      ...
      timelineRef.current.scrollLeft =
        (todayPosition / 100) * timelineRef.current.scrollWidth;
    }
  };
  ...
  const handleZoomFit = () => {
    if (timelineRef.current && ganttTasks.length > 0) {
      ...
      const earliestStart = Math.min(
        ...ganttTasks.map((t) => t.startDate.getTime()),
      );
      const latestEnd = Math.max(...ganttTasks.map((t) => t.endDate.getTime()));
      const projectSpan = differenceInDays(
        new Date(latestEnd),
  ...
  const getTaskPosition = (task: GanttTask) => {
    ...
    const effectiveWidth =
      viewMode === "months" ? taskDuration / 7 : taskDuration; // Weeks in month view
      ...
      <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg border-2 border-gray-200">
        <div className="text-center max-w-md mx-auto p-8">
          ...
          <p className="text-gray-600">
            Add tasks with due dates and dependencies to visualize your project
            timeline.
          </p>
          {debugInfo && (
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mt-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-orange-800 font-medium">
                {schedulingStats.conflictedTasks} task
                {schedulingStats.conflictedTasks !== 1 ? "s" : ""} automatically
                rescheduled due to dependencies
              </span>
            </div>
          </div>
        )}
        ...
        <div className="w-80 flex-shrink-0 bg-gray-50 border-r border-gray-200 flex flex-col">
          ...
          >
            {ganttTasks.map((task) => (
              <div
                key={`info-${task.id}`}
                className="px-4 py-5 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200 border-l-4 border-transparent hover:border-blue-400"
              >
                  ...
                  {task.assignee && (
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                        <span
                          className="text-xs text-gray-700 font-semibold truncate max-w-32"
                          title={task.assignee}
                        >
              ...
              <div className="flex h-10 border-b border-gray-300">
                {timelineHeaders.map((header) => (
                  <div
                    ...
                    style={{ width: `${100 / totalDays}%` }}
                  >
                    {header.isWeekStart && <div>{header.monthWeek}</div>}
                  </div>
                ))}
              </div>
              ...
              {ganttTasks.map((task, _taskIndex) => {
                ...
                return (
                  <div
                    ...
                    className="relative px-4 py-3 hover:bg-blue-50 transition-all duration-200 group border-b border-gray-100"
                    style={{ height: "60px" }}
                  >
                    {/* Today indicator */}
