...
const MeridianBadge = React.forwardRef<HTMLDivElement, MeridianBadgeProps>(
  (
    ...
    return (
      ...
      >
        {/* Status dot */}
        {dot && <span className="w-2 h-2 rounded-full bg-current opacity-75" />}
...
export interface PriorityBadgeProps
  extends Omit<MeridianBadgeProps, "variant"> {
...
const PriorityBadge = React.forwardRef<HTMLDivElement, PriorityBadgeProps>(
  ({ priority, ...props }, ref) => {
    ...
    return (
      <MeridianBadge ref={ref} variant={config.variant} {...props}>
        {config.children}
      </MeridianBadge>
    );
  },
...
export interface RoleBadgeProps
  extends Omit<MeridianBadgeProps, "variant" | "persona"> {
