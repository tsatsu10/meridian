...
function TimeTracker({
  ...
  const handleAddManual = async () => {
    ...
    const hours = Number.parseInt(duration.split(":")[0] || "0", 10);
    const minutes = Number.parseInt(duration.split(":")[1] || "0", 10);
  ...
  const handleEdit = async (id: string) => {
    ...
    const hours = Number.parseInt(editDuration.split(":")[0] || "0", 10);
    const minutes = Number.parseInt(editDuration.split(":")[1] || "0", 10);
    const totalSeconds = hours * 3600 + minutes * 60;
  ...
  const totalTime =
    entries.reduce((sum, entry) => sum + entry.duration, 0) + elapsed;
...
describe("TimeTracker", () => {
  ...
  it("should handle editing time entry", async () => {
    ...
    render(<TimeTracker entries={mockEntries} onEdit={onEdit} />, {
      wrapper: TestWrapper,
    });
  ...
  it("should handle deleting time entry", async () => {
    ...
    render(<TimeTracker entries={mockEntries} onDelete={onDelete} />, {
      wrapper: TestWrapper,
    });
  ...
  it("should update elapsed time when timer is running", async () => {
    ...
    render(<TimeTracker currentTimer={{ description: "Test", startTime }} />, {
      wrapper: TestWrapper,
    });
