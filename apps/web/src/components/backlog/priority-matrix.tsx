...
import { Grid, Maximize2, Minimize2, RotateCcw, Info } from "lucide-react";
...
export default function PriorityMatrix({
  ...
  const TaskCard = ({
    task,
  }: { task: TaskWithPriority; quadrant: MatrixQuadrant }) => (
    <div
      className={cn(
  ...
  const QuadrantSection = ({
    quadrant,
    config,
  }: { quadrant: MatrixQuadrant; config: QuadrantConfig }) => {
  ...
  const quadrantTotals = Object.entries(categorizedTasks).map(
    ([key, tasks]) => ({
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {quadrantTotals.map(
                ({ quadrant, count, storyPoints, config }) => (
                  <div
                    key={quadrant}
