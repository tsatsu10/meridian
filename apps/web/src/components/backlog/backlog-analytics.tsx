...
export default function BacklogAnalytics({
  ...
  void useMemo(() => {
    ...
    const refinedTasks = tasks.filter(
      (task) => task.refinementStatus === "refined",
    ).length;
    const readyTasks = tasks.filter(
      (task) => task.refinementStatus === "ready",
    ).length;
  ...
  void useMemo(() => {
    const tasksWithPoints = tasks.filter(
      (task) => task.storyPoints && task.storyPoints > 0,
    );
    ...
    const averagePoints =
      tasksWithPoints.length > 0 ? totalPoints / tasksWithPoints.length : 0;
  ...
  const analytics = useMemo(() => {
    ...
    const refinementDistribution = tasks.reduce(
      (acc, task) => {
    ...
    const assigneeDistribution = tasks.reduce(
      (acc, task) => {
        if (task.userEmail) {
  ...
  // Prepare chart data
  const priorityChartData = Object.entries(analytics.priorityDistribution).map(
    ([name, value]) => ({
      name,
      value,
    }),
  ...
  const refinementChartData = Object.entries(
    analytics.refinementDistribution,
  ).map(([name, value]) => ({
    name,
    value,
  }));
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="mt-4">
              <Progress
                value={
                  (analytics.completedPoints / analytics.totalStoryPoints) * 100
                }
                className="h-2"
              />
              <p className="text-sm text-muted-foreground mt-2">
