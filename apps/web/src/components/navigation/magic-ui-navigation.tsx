...
export const DockNavigation: React.FC<MagicUINavigationProps> = ({
  ...
  const allItems =
    items || navigationSections.flatMap((section) => section.items);
      ...
      <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50">
        ...
        >
          <Dock
            className={cn(
              enableGlassMorphism &&
                "glass-card border-border/50 dark:border-border/30 bg-white/80 dark:bg-black/50 backdrop-blur-xl shadow-2xl",
              className,
            )}
            onMouseMove={
  ...
  return (
    ...
    >
      {navigationSections.map((section) => (
        <div
          key={section.id}
          className={cn("space-y-1", !showLabels && "space-y-2")}
        >
          ...
          <div
            className={cn(
              "space-y-1",
              !showLabels && "flex flex-col items-center space-y-2",
            )}
          >
            {section.items.map((item) => {
              ...
              return <div key={item.id}>{renderNavigationItem(item)}</div>;
            })}
          </div>
        </div>
      ))}
