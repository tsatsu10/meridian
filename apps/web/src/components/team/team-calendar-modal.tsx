...
export default function TeamCalendarModal({
  ...
  const getEventsForTeam = () => {
    ...
    return events.filter((event) => {
      ...
      return event.attendees.some((attendee) =>
        team.members.find(
          (member) => member.id === attendee || member.name === attendee,
        ),
      );
    });
  };
  ...
  const renderCalendarView = () => {
    ...
    if (viewMode === "agenda") {
      ...
      const groupedEvents = teamEvents.reduce(
        (groups: Record<string, typeof teamEvents>, event) => {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {teamEvents.length}{" "}
                {teamEvents.length === 1 ? "event" : "events"} scheduled
              </p>
            </div>
  ...
  const handleIntegrationConfirm = () => {
    if (integrationAction === "export") {
      ...
      if (!clientId) {
        toast.error(
          "Google Calendar integration not configured. Please contact your administrator.",
        );
        return;
      }
      ...
      if (!clientId) {
        toast.error(
          "Outlook Calendar integration not configured. Please contact your administrator.",
        );
        return;
      }
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      ...
                      {hasConflicts && (
                        <Badge
                          variant="outline"
                          className="ml-4 shadow-lg animate-pulse border-red-500 bg-red-50 text-red-700"
                        >
                ...
                <>
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                Review and resolve conflicts to optimize your
                                schedule
                              </p>
                            </div>
                            <Badge
                        ...
                        <TabsContent value="timeline" className="mt-0">
                          {team ? (
                            <TimelineView
                              ...
                              onEntryClick={(entry) =>
                                toast.info(`Clicked: ${entry.title}`)
                              }
                          ...
                          {team ? (
                            <WorkloadHeatmap
                              ...
                              onCellClick={(date, member) => {
                                toast.info(
                                  `${member.memberName} on ${format(date, "MMM d")}`,
                                );
                              }}
                              className="bg-background/80 backdrop-blur-sm"
                            />
                            ...
                            <div className="flex flex-col items-center justify-center py-16 space-y-4">
                              ...
                              <div className="text-center space-y-2">
                                ...
                                <p className="text-sm text-muted-foreground max-w-md">
                                  Choose a team to view their workload
                                  distribution
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="space-y-3">
                              {Array.from({ length: 4 }, (_, week) => {
                                ...
                                const estimatedHours = weekEvents.reduce(
                                  (total: number, event: CalendarEvent) =>
                                    total + (event.estimatedHours || 0),
                                  0,
                                );
                                const workloadPercentage = Math.min(
                                  (estimatedHours / 40) * 100,
                                  100,
                                );
                  ...
                  <div className="mt-6 p-4 bg-muted/30 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      {Object.entries(eventTypeIcons).map(
                        ([type, IconComponent]) => (
                          <div
                            key={type}
            ...
            {integrationAction === "export" && (
              <div className="space-y-2">
                ...
                <ul className="text-sm text-muted-foreground list-disc list-inside space-y-1">
                  <li>
                    {getEventsForTeam().length} events from {team?.name}
                  </li>
                ...
                <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                  <p className="text-sm flex items-start gap-2">
                    ...
                    <span>
                      ...
                      {integrationAction === "google" ? "Google" : "Microsoft"}{" "}
