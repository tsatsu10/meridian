...
export default function TeamSettingsModalRedesign({
  ...
  const { data: integrationsData, isLoading: isLoadingIntegrations } =
    useGetTeamIntegrations(initialTeam?.id);
  ...
  const { data: automationsData, isLoading: isLoadingAutomations } =
    useGetTeamAutomations(initialTeam?.id);
  ...
  const filteredMembers = useMemo(() => {
    ...
    if (memberFilters.role) {
      filtered = filtered.filter(
        (member) => member.role === memberFilters.role,
      );
    }
                ...
                {activeTab === "automations" && (
                  <div className="space-y-6 max-w-5xl">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        onClick={() =>
                          toast.info("Automation creation coming soon!")
                        }
                      >
                        <Plus className="w-4 h-4 mr-2" />
                        Create Automation
                    ...
                    {isLoadingAutomations ? (
                      ...
                      automationsData.automations.length > 0 ? (
                      <div className="space-y-3">
