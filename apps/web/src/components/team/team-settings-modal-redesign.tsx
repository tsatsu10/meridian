...
const roleHierarchy = [
  {
    value: "Owner",
    label: "Owner",
    description: "Full access to all team settings",
  },
  {
    value: "Admin",
    label: "Admin",
  ...
  },
  {
    value: "Team Lead",
    label: "Team Lead",
    description: "Can assign tasks and view reports",
  },
  {
    value: "Member",
    label: "Member",
...
/** Map API/DB roles (e.g. "member", "lead") to UI select values. */
function toUiTeamRole(role: string): string {
  ...
  if (roleHierarchy.some((r) => r.value === role)) return role;
  return "Member";
}

/** Map UI select values to API/DB role strings. */
function toApiTeamRole(uiRole: string): string {
  switch (uiRole) {
    case "Owner":
      return "owner";
    case "Admin":
...
export default function TeamSettingsModalRedesign({
  ...
  const { data: statisticsData, isLoading: isLoadingStats } =
    useGetTeamStatistics(initialTeam?.id);
  const [activityPage, setActivityPage] = useState(0);
  const { data: activityData, isLoading: isLoadingActivity } =
    useGetTeamActivity(initialTeam?.id, {
      limit: 20,
      offset: activityPage * 20,
    });
  const { data: notificationsData, isLoading: isLoadingNotifications } =
    useGetTeamNotifications(initialTeam?.id);
  const [analyticsTimeRange, setAnalyticsTimeRange] = useState<
    "7d" | "30d" | "90d" | "all"
  >("7d");
  const { data: analyticsData, isLoading: isLoadingAnalytics } =
    useGetTeamAnalytics(initialTeam?.id, analyticsTimeRange);
  const {
    data: advancedPermissionsData,
    isLoading: isLoadingAdvancedPermissions,
  } = useGetAdvancedPermissions(initialTeam?.id);

  const { data: workspaceUsers = [] } = useGetWorkspaceUsers({
    workspaceId: initialTeam?.workspaceId ?? "",
  });
  ...
  const applyTeamUpdate = (next: Team) => {
    setTeam(next);
    // Keep Overview form fields, but never let editedTeam.members go stale.
    setEditedTeam((prev) => {
      ...
      if (isEditing) {
        return {
          ...
          // Keep name/description from the in-progress form.
        };
      }
      return next;
    });
  ...
  useEffect(() => {
    ...
    setTeam(initialTeam);
    // Don't clobber in-progress Overview name/description edits when parent
    // syncs members — but always refresh members on the form snapshot.
    setEditedTeam((prev) => {
      if (!isEditing || !prev || prev.id !== initialTeam.id) {
        return initialTeam;
      }
      return {
        ...prev,
  ...
  const addMemberCandidates = useMemo(() => {
    ...
    return workspaceUsers.filter(
      (user) => user.id != null && !onTeam.has(user.id),
    );
  }, [team, workspaceUsers]);
  ...
  const filteredMembers = useMemo(() => {
    ...
    if (memberFilters.role) {
      filtered = filtered.filter(
        (member) => toUiTeamRole(member.role) === memberFilters.role,
      );
    }
  ...
  const handleSaveChanges = async () => {
    if (!validateTeamForm() || !editedTeam || !team) return;
    ...
    updateTeamMutation.mutate(
      ...
      {
        onSuccess: () => {
          // Merge live members onto saved Overview fields so member
          // mutations during edit are not wiped.
          const saved: Team = {
            ...editedTeam,
  ...
  const handleChangeRole = async (memberId: string, newUiRole: string) => {
    if (!team) return;
        ...
        onSuccess: () => {
          applyTeamUpdate({
            ...
            members: team.members.map((m) =>
              m.id === memberId ? { ...m, role: newUiRole } : m,
            ),
          });
        },
      },
  ...
  return (
    <>
      ...
      >
        <DialogContent
          className="max-w-[98vw] w-[1400px] h-[95vh] p-0 gap-0 bg-gradient-to-br from-background via-background to-muted/20"
          hideOverlay={isAddMemberOpen}
          onInteractOutside={(event) => {
            if (isAddMemberOpen) event.preventDefault();
          }}
                              ...
                              <MagicCard className="relative p-6 bg-gradient-to-br from-blue-500/10 to-background border-blue-500/20">
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <p className="text-3xl font-bold mt-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-cyan-600">
                                      {statisticsData.statistics.memberCount}
                                    </p>
                                  </div>
                              ...
                              <MagicCard className="relative p-6 bg-gradient-to-br from-purple-500/10 to-background border-purple-500/20">
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <p className="text-3xl font-bold mt-2 bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-pink-600">
                                      {statisticsData.statistics.tasks.total}
                                    </p>
                                  </div>
                              ...
                              <MagicCard className="relative p-6 bg-gradient-to-br from-green-500/10 to-background border-green-500/20">
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <p className="text-3xl font-bold mt-2 bg-clip-text text-transparent bg-gradient-to-r from-green-600 to-emerald-600">
                                      {
                                        statisticsData.statistics.tasks
                                          .completionRate
                                      }
                                      %
                                    </p>
                              ...
                              <MagicCard className="relative p-6 bg-gradient-to-br from-orange-500/10 to-background border-orange-500/20">
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <p className="text-3xl font-bold mt-2 bg-clip-text text-transparent bg-gradient-to-r from-orange-600 to-amber-600">
                                      {
                                        statisticsData.statistics
                                          .recentActivityCount
                                      }
                        ...
                        <div className="space-y-6">
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {analyticsData.analytics.memberProductivity.map(
                                (member) => (
                                  <MagicCard
                                    key={member.memberId}
                                    className="p-4"
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <MagicCard className="p-6">
                              ...
                              <div className="space-y-3">
                                {analyticsData.analytics.statusDistribution.map(
                                  (item) => (
                                    <div
                                      key={item.status}
                            ...
                            <MagicCard className="p-6">
                              ...
                              <div className="space-y-3">
                                {analyticsData.analytics.priorityDistribution.map(
                                  (item) => (
                                    <div
                                      key={item.priority || "none"}
                                      className="flex items-center justify-between"
                                    >
                                      <span className="text-sm capitalize">
                                        {item.priority || "none"}
                          ...
                          {analyticsData.analytics.taskTrend.length > 0 && (
                            <MagicCard className="p-6">
                              ...
                              <div className="space-y-2">
                                {analyticsData.analytics.taskTrend
                                  .slice(-10)
                                  .map((item) => (
                                    <div
                                      key={item.date}
                                    ...
                                    >
                                      ...
                                      <div className="flex-1 flex gap-1">
                                        <div
                                          ...
                                          style={{
                                            width: `${Math.max(Number(item.completed) * 10, 2)}px`,
                                          }}
                                        ...
                                        <div
                                          ...
                                          style={{
                                            width: `${Math.max(Number(item.created) * 10, 2)}px`,
                                          }}
                          ...
                          filteredMembers.map((member) => (
                            <MagicCard key={member.id} className="p-4">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  <select
                                    value={toUiTeamRole(member.role)}
                                    onChange={(e) =>
                                      handleChangeRole(
                                        member.id,
                                        e.target.value,
                          ...
                          {activityData.pagination.total > 20 && (
                            <div className="flex items-center justify-between pt-4">
                              ...
                              <span className="text-sm text-muted-foreground">
                                ...
                                {Math.ceil(activityData.pagination.total / 20)}
                              </span>
                              <Button
                                variant="outline"
                                size="sm"
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                              Hide this team while preserving all data. Can be
                              restored later.
                            </p>
                          </div>
                          <Button
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                              Permanently delete this team. This action cannot
                              be undone.
                            </p>
                          </div>
                          <Button
                      ...
                      {showArchiveConfirm && (
                        <MagicCard className="p-6 bg-yellow-50 dark:bg-yellow-900/10">
                          ...
                          <p className="text-sm text-muted-foreground mb-4">
                            Are you sure you want to archive "{team.name}"? You
      ...
      >
        <DialogContent className="sm:max-w-md z-[60]">
          ...
          <DialogDescription>
            Choose someone already in this workspace to add to the team.
          </DialogDescription>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                const selectedUser = workspaceUsers.find(
