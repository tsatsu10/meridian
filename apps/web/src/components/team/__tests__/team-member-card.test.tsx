    ...
    >
      ...
      {member.tasksCompleted !== undefined && (
        <div
          ...
          aria-label={`${member.tasksCompleted} tasks completed`}
        >
      ...
      {member.currentTask && (
        <div
          ...
          aria-label={`Currently working on: ${member.currentTask}`}
        >
...
describe("TeamMemberCard", () => {
  ...
  it("should show current task", () => {
    ...
    expect(screen.getByLabelText(/currently working on/i)).toHaveTextContent(
  ...
  it("should render avatar placeholder when no avatar provided", () => {
    ...
    const { container } = render(<TeamMemberCard member={memberNoAvatar} />, {
      wrapper: TestWrapper,
    });

    expect(container.querySelector(".avatar-placeholder")).toBeInTheDocument();
    expect(container.querySelector(".avatar-placeholder")).toHaveTextContent(
      "JD",
    );
  });
  ...
  it("should hide actions when showActions is false", () => {
    render(<TeamMemberCard member={mockMember} showActions={false} />, {
      wrapper: TestWrapper,
    });

    expect(
      screen.queryByRole("group", { name: /member actions/i }),
  ...
  it("should be accessible", () => {
    ...
    expect(
      screen.getByRole("article", { name: /team member: john doe/i }),
    ).toBeInTheDocument();

    // Action group should have proper label
    expect(
      screen.getByRole("group", { name: /member actions/i }),
    ).toBeInTheDocument();
