    ...
    <div className="file-upload" data-testid="file-upload">
      ...
      >
        ...
        <div className="drop-zone-content">
          ...
          <p className="accepted-types">Accepted: {acceptedTypes.join(", ")}</p>
        </div>
      </div>
...
describe("FileUpload", () => {
  ...
  it("should display accepted file types", () => {
    render(<FileUpload acceptedTypes={["image/*", ".pdf"]} />, {
      wrapper: TestWrapper,
    });
  ...
  it("should handle multiple file selection", async () => {
    ...
    render(<FileUpload onUpload={onUpload} multiple={true} />, {
      wrapper: TestWrapper,
    });
  ...
  it("should enforce max files limit", async () => {
    ...
    const newFile = new File(["content"], "file3.pdf", {
      type: "application/pdf",
    });
    const input = screen.getByLabelText("File input");
  ...
  it("should show error status", () => {
    ...
    expect(screen.getByLabelText("Upload failed")).toHaveTextContent(
      "Network error",
    );
  });
  ...
  it("should handle file removal", async () => {
    ...
    render(<FileUpload uploadedFiles={files} onRemove={onRemove} />, {
      wrapper: TestWrapper,
    });
  ...
  it("should show file limit warning when limit reached", () => {
    ...
    render(<FileUpload maxFiles={2} uploadedFiles={files} />, {
      wrapper: TestWrapper,
    });

    expect(
      screen.getByText(/maximum file limit reached \(2 files\)/i),
    ).toBeInTheDocument();
  });
  ...
  it("should clear validation error on successful upload", async () => {
    ...
    render(<FileUpload maxSize={100} onUpload={onUpload} />, {
      wrapper: TestWrapper,
    });
