...
export default function BacklogTaskRow({ task, onTaskUpdate, backlogData, setBacklogData }: BacklogTaskRowProps) {
  ...
  const handleMoveToColumn = (columnId: string) => {
    ...
    setBacklogData(
      produce(backlogData, (draft) => {
        // Remove from archived tasks if present
        const archivedIndex = draft.archivedTasks.findIndex(
          (t) => t.id === task.id,
        );
        if (archivedIndex !== -1) {
          draft.archivedTasks.splice(archivedIndex, 1);
        }

        // Remove from planned tasks if present
        const plannedIndex = draft.plannedTasks.findIndex(
          (t) => t.id === task.id,
        );
        if (plannedIndex !== -1) {
          draft.plannedTasks.splice(plannedIndex, 1);
        }
            ...
            <div className="flex items-center space-x-2">
              {task.assignedTeamId ? (
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {task.assigneeName || task.userEmail}
                  </span>
                </div>
              ) : (
                <div className="flex items-center space-x-2">
