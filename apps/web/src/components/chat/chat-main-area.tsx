...
interface ChatMainAreaProps {
  ...
  /** Deep link: scroll to this message id after messages load */
  highlightMessageId?: string | null
  onHighlightConsumed?: () => void
}

/** Reactions / file attachments / edit-delete-pin are not fully backed by channel APIs + WS yet */
const CHAT_REACTIONS_ENABLED = false
const CHAT_FILE_ATTACHMENT_ENABLED = false
const CHAT_MESSAGE_MODERATION_ENABLED = false
...
export function ChatMainArea({
  ...
  useEffect(() => {
    if (!highlightMessageId || messagesLoading) return
    const timer = window.setTimeout(() => {
      const el = document.getElementById(`message-${highlightMessageId}`)
      if (el) {
        el.scrollIntoView({ behavior: 'smooth', block: 'center' })
        el.classList.add('ring-2', 'ring-blue-500', 'bg-blue-50', 'dark:bg-blue-500/10')
        window.setTimeout(() => {
          el.classList.remove('ring-2', 'ring-blue-500', 'bg-blue-50', 'dark:bg-blue-500/10')
          onHighlightConsumed?.()
        }, 2000)
      } else {
        onHighlightConsumed?.()
  ...
  }, [highlightMessageId, messagesLoading, messages.length, selectedChatId, onHighlightConsumed])
  ...
  const { data: channelInfo } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`${API_BASE_URL}/channel/${selectedChatId}`, {
        credentials: 'include',
      })
      if (!response.ok) {
  ...
  const unifiedWS = useUnifiedWebSocket({
    ...
    onTyping: (user) => {
      logger.debug(`${user.userEmail} is typing...`);
    },
    onError: (error) => {
      ...
      if (channelId && workspace?.id) {
        try {
          ...
          if (response.ok) {
            ...
            logger.error('Failed to join channel', { status: response.status, detail });
            setAccessDenied(true);
          }
        } catch (error) {
  ...
  const handleEdit = (message: Message) => {
    ...
    logger.debug('Edit message:', message)
  }
  ...
  const handlePin = (messageId: string) => {
    ...
    logger.debug('Pin message:', messageId)
  }
  ...
  const handleMentionsKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    const filteredUsers = workspaceUsers.filter((u: { name?: string; email?: string }) => {
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2 mb-1">
            <div 
              ...
              onClick={() => {
                // ✅ FIX: Actually call onSelectUser when clicking on a user
                if (onSelectUser) {
                  onSelectUser({
                    id: message.userId,
                    ...
                    avatar: `https://avatar.vercel.sh/${message.userEmail}`,
                    userAvatar: `https://avatar.vercel.sh/${message.userEmail}`,
                  });
                }
              }}
            >
              <ClickableUserProfile
                ...
                showName={true}
                showAvatar={true}
                nameClassName="font-semibold text-sm text-slate-900 dark:text-white hover:underline"
              />
            </div>
          ...
          {CHAT_REACTIONS_ENABLED && (messageReactions[message.id]?.length ?? 0) > 0 && (
          ...
          <div className="flex items-center gap-2 mt-2 relative">
            ...
            {CHAT_MESSAGE_MODERATION_ENABLED && (
              <DropdownMenu>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-shrink-0 p-6 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        ...
        {(!CHAT_REACTIONS_ENABLED || !CHAT_FILE_ATTACHMENT_ENABLED || !CHAT_MESSAGE_MODERATION_ENABLED) && (
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
            Reactions, file attachments, and message moderation are not available in this build yet.
          </p>
        )}
      </div>
      ...
      {/* File Upload Modal */}
      <FileUploadModal
        isOpen={showFileUpload && CHAT_FILE_ATTACHMENT_ENABLED}
