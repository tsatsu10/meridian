...
export function LiveCommentThread({
  ...
  useEffect(() => {
    ...
    logger.debug('💬 Listening for real-time comment updates on task:', taskId);

    return () => {
      logger.debug('💬 Stopped listening for comment updates on task:', taskId);
    };
  }, [isConnected, taskId]);
  ...
  const handleSubmitComment = () => {
    if (newComment.trim()) {
      ...
      logger.debug('💬 New comment posted:', { taskId, content: newComment });
    }
  };
  ...
  const handleSaveEdit = () => {
    if (editingCommentId && editContent.trim()) {
      ...
      logger.debug('💬 Comment edited:', { commentId: editingCommentId, content: editContent });
    }
  };
...
export function useCommentReactions(commentId: string) {
  ...
  const addReaction = (emoji: string) => {
    ...
    logger.debug('😀 Adding reaction:', { commentId, emoji });
  ...
  const removeReaction = (emoji: string) => {
    ...
    logger.debug('😐 Removing reaction:', { commentId, emoji });
    // This would send a WebSocket message
  };
