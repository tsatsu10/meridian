...
export default function MilestoneDashboard({
  ...
  const realMilestones = useMemo(() => {
    if (passedMilestones && passedMilestones.length > 0) {
      ...
      return passedMilestones.map((milestone) => ({
        ...
        priority: (milestone.riskLevel === "critical"
          ? "critical"
          : milestone.riskLevel === "high"
            ? "high"
            : milestone.riskLevel === "medium"
              ? "medium"
              : "low") as Milestone["priority"],
  ...
  const metrics = useMemo(() => {
    ...
    const usingDerivedMilestones =
      !passedMilestones || passedMilestones.length === 0;
    ...
    if (passedStats && !usingDerivedMilestones) {
      return {
        ...
        completionRate:
          passedStats.total > 0
            ? Math.round((passedStats.achieved / passedStats.total) * 100)
            : 0,
        avgProgress:
          passedStats.achieved > 0
            ? Math.round((passedStats.achieved / passedStats.total) * 100)
            : 0,
    ...
    const completed = filteredMilestones.filter(
      (m) => m.status === "completed",
    ).length;
    const overdue = filteredMilestones.filter(
      (m) => m.status === "overdue",
    ).length;
    const atRisk = filteredMilestones.filter(
      (m) => m.status === "at-risk",
    ).length;
    ...
    const avgHealth =
      total > 0
        ? Math.round(
            filteredMilestones.reduce((sum, m) => sum + m.healthScore, 0) /
              total,
          )
        : 100;
  ...
  const getUniqueProjects = () => {
    const projects = [
      ...new Set(
        realMilestones.map((m) => ({ id: m.projectId, name: m.projectName })),
      ),
    ];
    return projects;
  };
          ...
          {metrics.overdue > 0 && (
            <div className="flex items-center gap-2 p-2 bg-red-50 rounded border border-red-200">
              ...
              <span className="text-sm text-red-700">
                {metrics.overdue} overdue milestones
              </span>
            </div>
          )}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={cn(
                    "text-2xl font-bold",
                    getHealthColor(metrics.avgHealth),
                  )}
                >
                  {metrics.avgHealth}%
                </p>
                ...
                <div className="flex items-center gap-2">
                  <div className="text-right mr-4">
                    ...
                    <div
                      className={cn(
                        "text-xs",
                        getHealthColor(milestone.healthScore),
                      )}
                    >
              ...
              {milestone.tasks.length > 0 && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span className="text-blue-600">
                    {
                      milestone.tasks.filter((t) => t.status === "in_progress")
                        .length
                    }{" "}
                    in progress
                  </span>
