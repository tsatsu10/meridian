...
describe("RiskAlertSection", () => {
  it("renders nothing when there are no alerts", () => {
    ...
    const { container } = render(
      <RiskAlertSection riskData={mockRiskDataWithoutAlerts} />,
      { wrapper: Wrapper },
    );
  ...
  it("renders nothing when risk data is null", () => {
    const Wrapper = createWrapper();
    const { container } = render(
      <RiskAlertSection riskData={mockRiskDataNull} />,
      { wrapper: Wrapper },
    );
    expect(container.firstChild).toBeNull();
  });
  ...
  it("renders the section when alerts are present", () => {
    const Wrapper = createWrapper();
    const { container } = render(
      <RiskAlertSection riskData={mockRiskDataWithAlerts} />,
      { wrapper: Wrapper },
    );
  ...
  it("displays only the first 3 alerts when more than 3 exist", () => {
    const Wrapper = createWrapper();
    render(<RiskAlertSection riskData={mockRiskDataWithAlerts} />, {
      wrapper: Wrapper,
    });
  ...
  it("displays correct alert information", () => {
    ...
    expect(screen.getByText("High CPU Usage")).toBeInTheDocument();
    expect(
      screen.getByText("CPU usage is above 90% threshold"),
    ).toBeInTheDocument();
  ...
  it("applies correct styling and classes", () => {
    ...
    const cardElement = screen
      .getByText("Risk Detection System")
  ...
  it("handles edge case with single alert", () => {
    ...
    const Wrapper = createWrapper();
    const { container } = render(
      <RiskAlertSection riskData={singleAlertData} />,
      { wrapper: Wrapper },
    );
  ...
  it("handles alerts with empty affected tasks array", () => {
    ...
    const Wrapper = createWrapper();
    render(<RiskAlertSection riskData={alertWithNoTasks} />, {
      wrapper: Wrapper,
    });
  ...
  it("generates unique keys for alerts correctly", () => {
    ...
    const alertContainers = screen
      .getAllByText(/tasks affected/)
