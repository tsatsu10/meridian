...
const TaskCard = React.memo(
  function TaskCard({ task, hierarchyLevel = 0, parentTask }: TaskCardProps) {
    ...
    const hierarchyBorderClass =
      hierarchyLevel > 0
        ? "border-l-2 border-l-indigo-400 dark:border-l-indigo-500"
        : "";

    const hierarchyBgClass =
      hierarchyLevel > 0 ? "bg-indigo-50/30 dark:bg-indigo-950/30" : "bg-card";
                ...
                <div className="flex items-center gap-1.5">
                  {isNestedSubtask && (
                    <span
                      ...
                      title={`Subtask of ${parentTask?.title || "parent task"}`}
                    >
                      ↳
                    </span>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    {task.subtasks && task.subtasks.length > 0 && (
                      ...
                      >
                        ...
                        <span
                          className={`text-xs ml-0.5 font-semibold px-1 py-0.5 rounded min-w-[16px] text-center ${
                            isExpanded
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      JSON.stringify(prevProps.task.dependencies) ===
        JSON.stringify(nextProps.task.dependencies) &&
      JSON.stringify(prevProps.task.blockedBy) ===
        JSON.stringify(nextProps.task.blockedBy)
    );
  },
);
