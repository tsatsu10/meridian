    ...
    if (incompleteBlockers.length > 0) {
      const blockerNames = incompleteBlockers
        .map((dep: any) => {
          ...
          return requiredTask
            ? `${projectSlug}-${requiredTask.number}`
            : "Unknown";
        })
        .join(", ");
      ...
      const dependentNames = dependentTasks
        .map((dep: any) => {
          ...
          return dependentTask
            ? `${projectSlug}-${dependentTask.number}`
            : "Unknown";
        })
        .join(", ");
...
function KanbanBoard({ project }: KanbanBoardProps) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (destinationColumn) {
      // Only update if actually changing columns
      if (sourceColumn.id !== destinationColumn.id) {
        // @epic-1.2-dependencies: Validate task movement based on dependencies
        const validation = validateTaskMove(
        ...
        const allChildren = findAllChildrenOfTask(draggedTask.id, allTasks);
        ...
        tasksToUpdate.forEach((taskToUpdate, index) => {
          ...
          updateTask(updatedTask, {
            onError: (error: any) => {
              ...
              toast.error(
                `Failed to move "${updatedTask.title}" to ${destinationColumn.name}. Please try again.`,
              );
            },
            onSuccess: () => {
              if (index === 0) {
                // Only show success message for parent task
                ...
                const message =
                  subtaskCount > 0
                    ...
                    : `Moved "${draggedTask.title}" to ${destinationColumn.name}`;
                    ...
                    >
                      <div className="relative group">
                        ...
                        <div className="relative bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border border-zinc-200/50 dark:border-zinc-800/50 rounded-xl shadow-lg shadow-zinc-200/20 dark:shadow-zinc-900/20 hover:shadow-xl hover:shadow-zinc-200/30 dark:hover:shadow-zinc-900/30 transition-all duration-300">
                          <Column
                            column={
                              column as ProjectWithTasks["columns"][number]
                            }
                          />
                        </div>
                      </div>
                    ...
                    <div className="text-center space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-sm">
                          Create your first column to start organizing tasks in
                          your Kanban board.
                        </p>
                      </div>
                    </div>
