...
export function MessageArea({
  ...
  const handleEdit = (message: Message) => {
    ...
    logger.debug('Edit message:', message.id);
  };
  ...
  const handleReaction = (messageId: string, emoji: string) => {
    ...
    logger.debug('React to message:', messageId, emoji);
  };
        ...
        {isFirstInGroup && (
          <div className="flex items-center space-x-2 mb-1">
            <ClickableUserProfile
              ...
              userName={message.userName || message.userEmail.split('@')[0]}
              userAvatar={`https://api.dicebear.com/7.x/initials/svg?seed=${message.userEmail}`}
              size="sm"
              openMode="both"
