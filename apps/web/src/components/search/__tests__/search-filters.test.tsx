...
function SearchFilters({
  ...
  const [filters, setFilters] =
    React.useState<Record<string, string[]>>(activeFilters);
        ...
        {group.options.map((option) => (
          <label key={option.id} className="filter-option">
            <input
              ...
              onChange={() =>
                handleFilterToggle(group.id, option.value, group.type)
              }
  ...
  return (
    <div className="search-filters" data-testid="search-filters">
      ...
      {/* Filter Groups */}
      <div className="filter-groups">{filterGroups.map(renderFilterGroup)}</div>
...
describe("SearchFilters", () => {
  ...
  it("should display result count", () => {
    render(<SearchFilters filterGroups={mockFilterGroups} resultCount={42} />, {
      wrapper: TestWrapper,
    });
  ...
  it("should show save filter button when filters are active", async () => {
    ...
    expect(
      screen.queryByLabelText(/save current filters/i),
    ...
    await waitFor(() => {
      expect(
        screen.getByLabelText(/save current filters/i),
      ).toBeInTheDocument();
    });
  });
