...
export function PermissionDebug() {
  const { user, getAllowedActions, currentWorkspace, currentProject } =
    useRBACAuth();
  ...
  return (
    <details className="permission-debug">
      ...
      <div>
        ...
        <div>
          <strong>Workspace:</strong> {currentWorkspace || "None"}
        </div>
        <div>
          <strong>Project:</strong> {currentProject || "None"}
        </div>
        <div>
          <strong>Allowed Actions ({allowedActions.length}):</strong>
        </div>
