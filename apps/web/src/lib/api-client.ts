...
class SmartApiClient {
  ...
  constructor() {
    ...
    logger.debug("🤖 Smart API Client initialized:", {
  ...
  workspaces = {
    ...
    get: async (id: string) => {
      ...
      const response = await rpc.workspace[":workspaceId"].$get({
  ...
  projects = {
    ...
    get: async (id: string) => {
      ...
      const response = await rpc.project[":projectId"].$get({
        param: { projectId: id },
      });
    ...
    create: async (workspaceId: string, data: any) => {
      ...
      const response = await rpc.project.$post({
    ...
    teams: {
      ...
      addMember: async (projectId: string, teamId: string, data: any) => {
        ...
        return {
          success: true,
          member: { id: `member-${Date.now()}`, ...data },
        };
      },
      ...
      removeMember: async (
        ...
        if (this.useLive) {
          return liveApi.projects.teams.removeMember(
            projectId,
            teamId,
            memberId,
          );
  ...
  tasks = {
    list: async (projectId: string) => {
      ...
      const response = await rpc.task.tasks[":projectId"].$get({
        param: { projectId },
      });
    ...
    get: async (id: string) => {
      ...
      const response = await rpc.task[":taskId"].$get({
        param: { taskId: id },
      });
  ...
  analytics = {
    ...
    notifications: async (
      params: { timeRange?: string; workspaceId?: string } = {},
    ) => {
      if (this.useLive) {
      ...
      return {
        ...
        hourlyData: Array.from({ length: 24 }, (_, hour) => ({
          hour,
          notifications:
            Math.floor(Math.random() * 10) + (hour >= 9 && hour <= 17 ? 3 : 0),
  ...
  integrations = {
    ...
    connect: async (integrationId: string, config: any) => {
      ...
      return {
        success: true,
        integration: { id: integrationId, status: "connected", ...config },
      };
    },
  ...
  forceLiveMode() {
    this.useLive = true;
    logger.debug("🔄 Switched to live mode");
  }
  ...
  forceMockMode() {
    this.useLive = false;
    logger.debug("🔄 Switched to mock mode");
  }
