      ...
      if (schedule.workload > 90) {
        ...
        if (overloadEvents.length > 0) {
          detectedConflicts.push({
            ...
            suggestedResolutions: generateOverloadResolutions(
              schedule,
              overloadEvents,
            ),
          });
      ...
      event.attendees.forEach((memberId) => {
        ...
        if (
          ...
          (schedule.availability === "unavailable" ||
            schedule.availability === "out-of-office")
        ) {
          detectedConflicts.push({
            ...
            suggestedResolutions: generateAvailabilityResolutions(
              event,
              schedule,
            ),
          });
...
function generateOverlapResolutions(
...
function generateOverloadResolutions(
  _schedule: MemberSchedule,
  events: CalendarEvent[],
): Resolution[] {
...
function generateAvailabilityResolutions(
  event: CalendarEvent,
