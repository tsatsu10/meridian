...
export function useScheduleDragDrop({
  ...
  const updateDragTarget = useCallback(
    (targetDate: Date, targetMember?: string) => {
  ...
  const startResize = useCallback(
    (event: CalendarEvent, direction: "start" | "end") => {
      setIsResizing(true);
  ...
  const moveEventByDays = useCallback(
...
function validateDrop(
  ...
  if (targetMember) {
    const memberSchedule = memberSchedules.find(
      (s) => s.memberId === targetMember,
    );
    if (!memberSchedule) return false;

    // Check if member is available
    if (
      memberSchedule.availability === "unavailable" ||
