  ...
  const suggestions = useMemo(() => {
    ...
    imbalances.forEach((imbalance, index) => {
      smartSuggestions.push({
        ...
        affectedMembers: [
          imbalance.overloadedMember,
          imbalance.underutilizedMember,
        ],
        confidence: 85,
      ...
      if (schedule.workload > 85 && schedule.hoursScheduled > 6) {
        ...
        if (breakSuggestion) {
          smartSuggestions.push({
            ...
            confidence: 90,
            reasoning:
              "Continuous work without breaks leads to decreased productivity and increased burnout risk.",
          });
        }
      }
    });
  ...
  const suggestionStats = useMemo(
    () => ({
      ...
      byType: {
        ...
        loadBalance: suggestions.filter((s) => s.type === "load-balance")
          .length,
        breakReminder: suggestions.filter((s) => s.type === "break-reminder")
          .length,
        deadlineRisk: suggestions.filter((s) => s.type === "deadline-risk")
          .length,
        resourceConflict: suggestions.filter(
          (s) => s.type === "resource-conflict",
        ).length,
      },
...
function findBestMeetingTimes(memberSchedules: MemberSchedule[]) {
  ...
  return times.sort((a, b) => b.availableCount - a.availableCount).slice(0, 3);
}
...
function findBreakTime(schedule: MemberSchedule) {
  ...
  for (let i = 0; i < sortedEvents.length - 1; i++) {
    const gap =
      sortedEvents[i + 1].startDate.getTime() -
      sortedEvents[i].endDate.getTime();
