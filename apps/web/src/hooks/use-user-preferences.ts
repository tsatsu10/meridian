...
export function useUserPreferences() {
  ...
  const loadFromLocalStorage = () => {
    try {
      const cached: UserPreferences = {
        pinnedProjects: JSON.parse(
          localStorage.getItem(STORAGE_KEYS.PINNED_PROJECTS) || "[]",
        ),
        settings: {
    ...
    try {
      if (prefs.pinnedProjects) {
        localStorage.setItem(
          STORAGE_KEYS.PINNED_PROJECTS,
          JSON.stringify(prefs.pinnedProjects),
        );
      }
      if (prefs.settings?.projectsViewMode) {
        localStorage.setItem(
          STORAGE_KEYS.PROJECTS_VIEW,
          prefs.settings.projectsViewMode,
        );
      }
      if (prefs.settings?.allTasksViewMode) {
        localStorage.setItem(
          STORAGE_KEYS.TASKS_VIEW,
          prefs.settings.allTasksViewMode,
        );
      }
      if (prefs.dashboardLayout) {
        localStorage.setItem(
          STORAGE_KEYS.DASHBOARD_LAYOUT,
          JSON.stringify(prefs.dashboardLayout),
        );
      }
      if (prefs.settings) {
        localStorage.setItem(
  ...
  const savePreferences = useCallback(
      ...
      saveTimerRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (Object.keys(sanitizedUpdates).length === 0) {
            console.warn(
              "[Preferences] No valid data to save, skipping API call",
            );
            return;
          }
          ...
          try {
            ...
            logger.debug("[Preferences] JSON body length:", {
              value: bodyString.length,
            });
            logger.debug("[Preferences] JSON body preview:", {
              value: bodyString.substring(0, 200),
            });
            ...
            console.error(
              "[Preferences] Failed to stringify/validate request body:",
              stringifyError,
            );
          ...
          logger.debug("[Preferences] Fetch completed, status:", {
            value: response.status,
          });

          logger.debug("[Preferences] Response status:", {
            value: response.status,
          });
    ...
    [user?.email],
  ); // Removed preferences and isSyncing from dependencies

  // Update specific preference fields (using refs to avoid stale closures)
  const preferencesRef = useRef(preferences);
  preferencesRef.current = preferences;

  const updateProjectsViewMode = useCallback(
    (mode: "grid" | "list" | "board") => {
      const newSettings = {
  ...
  const updateAllTasksViewMode = useCallback(
    (mode: "list" | "kanban" | "calendar") => {
      const newSettings = {
        ...preferencesRef.current.settings,
        allTasksViewMode: mode,
      };
      savePreferences({ settings: newSettings });
    },
    [savePreferences],
  ...
  const updateDashboardLayout = useCallback(
    (layout: Partial<DashboardSettings>) => {
      const newLayout = {
        ...preferencesRef.current.dashboardLayout,
        ...layout,
      };
