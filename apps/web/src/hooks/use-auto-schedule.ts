...
export function useAutoSchedule(tasks: TaskWithSubtasks[]) {
  const scheduledTasks = useMemo(() => {
    ...
    for (const task of tasks) {
      ...
      const originalStart = task.dueDate
        ? new Date(task.dueDate)
        : new Date(task.createdAt);
      const originalEnd = task.dueDate
        ? new Date(task.dueDate)
        : addDays(originalStart, defaultDuration);
      const duration = Math.max(
        1,
        differenceInDays(originalEnd, originalStart),
      );
  ...
  const schedulingStats = useMemo(() => {
    ...
    const conflictedTasks = scheduledTasks.filter(
      (t) => t.hasSchedulingConflict,
    ).length;
    const delayedTasks = scheduledTasks.filter(
      (t) => t.originalStartDate && t.scheduledStartDate > t.originalStartDate,
    ).length;
    ...
    const projectStart =
      scheduledTasks.length > 0
        ? new Date(
            Math.min(
              ...scheduledTasks.map((t) => t.scheduledStartDate.getTime()),
            ),
          )
        : new Date();
    const projectEnd =
      scheduledTasks.length > 0
        ? new Date(
            Math.max(
              ...scheduledTasks.map((t) => t.scheduledEndDate.getTime()),
            ),
          )
        : new Date();
  ...
  return {
    ...
    getConflictedTasks: () =>
      scheduledTasks.filter((t) => t.hasSchedulingConflict),
    getDelayedTasks: () =>
