  ...
  const timelineData = useMemo(() => {
    ...
    return projectList.map((project: { id: string; name: string }) => {
      ...
      const teamImpacts: TeamImpact[] = teams.map((team) => {
        const teamEvents = projectEvents.filter((event) =>
          event.impactedTeams?.includes(team.id),
        );
      ...
      const overCapacityTeams = teamImpacts.filter(
        (t) => t.capacityRisk === "high",
      );
      ...
      const totalProgress = projectEvents.reduce(
        (sum, event) => sum + event.progress,
        0,
      );
      const actualProgress =
        projectEvents.length > 0 ? totalProgress / projectEvents.length : 0;

      // Estimate completion date based on current progress
      const completedEvents = projectEvents.filter(
    ...
    return (teamId: string, days = 30) => {
      ...
      for (let week = 0; week < Math.ceil(days / 7); week++) {
        ...
        const weeklyHours = weekEvents.reduce(
          (total, event) => total + (event.estimatedHours || 0),
          0,
        );
