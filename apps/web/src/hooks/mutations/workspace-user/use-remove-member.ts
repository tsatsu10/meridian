...
export function useRemoveMember() {
  ...
  return useMutation({
    ...
    onMutate: async ({ workspaceId, memberId }) => {
      ...
      const previousWorkspaceUsers = queryClient.getQueryData([
        "workspace-users",
        workspaceId,
      ]);
    ...
    onError: (error: Error, variables, context) => {
      ...
      if (context?.previousWorkspaceUsers) {
        queryClient.setQueryData(
          ["workspace-users", variables.workspaceId],
          context.previousWorkspaceUsers,
        );
      }
    ...
    onSuccess: (data, variables) => {
      ...
      invalidateDashboardQueriesForWorkspace(
        queryClient,
