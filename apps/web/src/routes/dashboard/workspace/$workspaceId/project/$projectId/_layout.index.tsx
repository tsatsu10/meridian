...
import LazyDashboardLayout, {
  StatsCardSkeleton,
  ChartSkeleton,
  TableSkeleton,
} from "@/components/performance/lazy-dashboard-layout";
...
function ProjectOverview() {
  ...
  const projectStats: ProjectStats = useMemo(() => {
    ...
    const healthScore = Math.max(
      ...
      Math.min(
        100,
        Math.round((progressRatio * 0.6 - overdueRatio * 0.4) * 100 + 40),
      ),
    );
    ...
    if (riskLevel === "critical")
      adjustedHealthScore = Math.min(adjustedHealthScore, 30);
    else if (riskLevel === "high")
      adjustedHealthScore = Math.min(adjustedHealthScore, 50);
    else if (riskLevel === "medium")
      adjustedHealthScore = Math.min(adjustedHealthScore, 70);
      ...
      .map((member: any) => ({
        ...
        productivity:
          member.activeTasks + member.completedTasks > 0
            ? Math.round(
                (member.completedTasks /
                  (member.activeTasks + member.completedTasks)) *
                  100,
              )
            : 0,
      }))
    ...
    return allTasks
      ...
      .map((task: any) => ({
        ...
        type:
          task.status === "done"
            ? "task_completed"
            : ("task_created" as ActivityItem["type"]),
  ...
  const handleExportProject = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/projects/${projectId}/export?workspaceId=${workspaceId}`,
        {
          method: "POST",
          headers: {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/projects/${projectId}?workspaceId=${workspaceId}`,
        {
          method: "DELETE",
          credentials: "include",
        },
      ...
      if (result.deletionSummary) {
        logger.debug("🗑️ Deletion Summary:", result.deletionSummary);
        toast.info(
            ...
            <CardContent className="space-y-4">
              ...
              <Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950/20">
                ...
                <AlertDescription className="text-sm text-blue-700 dark:text-blue-300">
                  If you believe you should have access to this project, please
                  contact your workspace administrator or project manager.
                </AlertDescription>
              </Alert>
                ...
                <div>
                  ...
                  <p className="text-lg text-muted-foreground">
                    {projectData?.description ||
                      "Monitor your project progress and team performance"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-green-600 dark:text-green-400">
                    {Math.round(
                      (projectStats.completedTasks /
                        Math.max(projectStats.totalTasks, 1)) *
                        100,
                    )}
                    % completion rate
                  </p>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-purple-600 dark:text-purple-400">
                    {projectStats.healthScore >= 80
                      ? "Excellent"
                      : projectStats.healthScore >= 60
                        ? "Good"
                        : "Needs attention"}
                  </p>
                </div>
          ...
          <div className="lg:col-span-3 space-y-6">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {userTasks.length} tasks • {workloadPercentage}%
                            complete
                          </p>
                        </div>
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="space-y-3">
                ...
                {(projectPermissions.canManageTeam ||
                  projectPermissions.canInviteMembers) && (
                  <Button
                    variant="outline"
                    className="w-full justify-start h-12"
                    onClick={() => setIsInviteModalOpen(true)}
