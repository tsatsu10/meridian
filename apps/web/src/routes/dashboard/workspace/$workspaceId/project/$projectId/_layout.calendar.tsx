    ...
    for (const task of allTasks as CalendarTask[]) {
      if (task.dueDate) {
        try {
          ...
          console.error(
            "Calendar: Error parsing due date for task:",
            task.title,
            error,
          );
        }
  ...
  const filteredEvents = useMemo(() => {
    return calendarEvents.filter((event) => {
      ...
      if (
        ...
        event.assignee?.id !== filters.assignee
      ) {
        return false;
      }
        ...
        <div className="grid grid-cols-8 border-b border-border">
          ...
          {weekDays.map((day) => {
            const allDayEvents = filteredEvents.filter(
              (event) => isSameDay(event.start, day) && event.isAllDay,
            );
          ...
          upcomingEvents.map((event) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-muted-foreground">
                  {format(event.start, "MMM d, yyyy")} at{" "}
                  ...
                  >
                    <StatusIcon
                      className={cn(
                        ...
                        member.status === "partially-available" &&
                          "text-yellow-500",
                      )}
                    />
