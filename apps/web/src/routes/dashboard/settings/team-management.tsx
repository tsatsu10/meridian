...
const fetchTeamMembers = async (): Promise<TeamMember[]> => {
  try {
    ...
    const uniqueMembers = members.reduce(
      (acc: TeamMember[], member: TeamMember) => {
...
const updateMemberRole = async (
  ...
  const response = await fetch(
    `${API_BASE_URL}/rbac/assignments/${userId}/role`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
...
function TeamManagementSettings() {
  ...
  const handleCopyInviteLink = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/workspace/invites/generate-link`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const adminMembers = teamMembers.filter((m) =>
    ["workspace-manager", "department-head", "project-manager"].includes(
      m.role,
    ),
  ).length;
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {totalMembers === 0
                ? "No team members yet"
                : `${totalMembers} team member${totalMembers === 1 ? "" : "s"} total`}
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {teamMembers.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Start building your team by inviting colleagues to collaborate
                  on your projects.
                </p>
                <Button onClick={() => setIsInviteModalOpen(true)}>
                  <UserPlus className="h-4 w-4 mr-2" />
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {member.role !== "workspace-manager" && (
                            ...
                            >
                              ...
                              <SelectContent>
                                <SelectItem value="workspace-viewer">
                                  Workspace Viewer
                                </SelectItem>
