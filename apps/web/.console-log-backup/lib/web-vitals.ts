...
export function logPerformanceMetrics() {
  ...
  getPerformanceMetrics().then(metrics => {
    if (metrics.navigation) {
      ...
      console.log('DOM Interactive:', `${nav.domInteractive - nav.fetchStart}ms`);
      console.log('DOM Complete:', `${nav.domComplete - nav.fetchStart}ms`);
      console.log('Load Complete:', `${nav.loadEventEnd - nav.fetchStart}ms`);
      console.groupEnd();
    }

    if (metrics.memory) {
      console.group('💾 Memory Usage');
      console.log('Used:', `${(metrics.memory.usedJSHeapSize / 1048576).toFixed(2)} MB`);
      console.log('Total:', `${(metrics.memory.totalJSHeapSize / 1048576).toFixed(2)} MB`);
      console.log('Limit:', `${(metrics.memory.jsHeapSizeLimit / 1048576).toFixed(2)} MB`);
      console.groupEnd();
    }
  });
}
