    ...
    const loadPreferences = async () => {
      try {
        const response = await fetch(
          `${API_URL}/api/user-preferences?userEmail=${encodeURIComponent(user.email)}`,
          { credentials: 'include' }
        );
  ...
  const savePreferences = useCallback(async (updates: Partial<UserPreferences>) => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        console.log('[Preferences] Saving to API:', requestBody);
        console.log('[Preferences] Updates keys:', Object.keys(sanitizedUpdates));
        ...
        try {
          ...
          console.log('[Preferences] JSON body length:', bodyString.length);
          console.log('[Preferences] JSON body preview:', bodyString.substring(0, 200));
          ...
          console.log('[Preferences] JSON validation: PASSED');
        ...
        console.log('[Preferences] About to send fetch request...');
        ...
        console.log('[Preferences] Fetch completed, status:', response.status);
        ...
        console.log('[Preferences] Successfully saved:', saved);
